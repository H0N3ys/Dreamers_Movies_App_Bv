...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> {
  ...
  Future<void> _loadExtraData() async {
    await Future.wait([_fetchTmdbExtraDetails(), _loadReviewsByMovie()]);
  }
...
// 🔥 BUSCADOR DEL PERFIL ACTUAL 🔥
  Future<int?> _getCurrentProfileId() async {
    ...
    final profileResult = await db.query(
      'perfil',
      where: 'id_usuario = ?',
      whereArgs: [int.parse(userIdString)],
      limit: 1,
    );
    
    if (profileResult.isNotEmpty) {
      return profileResult.first['id_perfil'] as int;
    }
    return null;
  }
  Future<void> _checkIfFavorite() async {
    final idPerfil = await _getCurrentProfileId();
    if (idPerfil == null) return;

    final dbHelper = DatabaseHelper();
    final isFav = await dbHelper.isFavoriteLocal(idPerfil, widget.movie.id);
  ...
  Future<void> _toggleFavorite() async {
    ...
    if (_isFavorite) {
      // Si ya era favorita, la borramos
      await dbHelper.removeFavoriteLocal(idPerfil, widget.movie.id);
      if (mounted) _showFavoriteOverlay(isBreaking: true);
    } else {
      // Si no era favorita, la guardamos con todos sus datos
      await dbHelper.addFavoriteLocal(
        idPerfil: idPerfil,
        ...
        genero:
            _getGenreName(), // Usamos tu función para obtener el nombre exacto
      );
...
// 🔥 NUEVA VERIFICACIÓN DE GUARDADOS EN SQLITE 🔥
  Future<void> _checkIfSaved() async {
    final idPerfil = await _getCurrentProfileId();
    if (idPerfil == null) return;
    
    final dbHelper = DatabaseHelper();
    final saved = await dbHelper.isSavedLocal(idPerfil, widget.movie.id);
 ...
 // 🔥 NUEVA FUNCIÓN PARA AGREGAR/QUITAR GUARDADOS 🔥
  Future<void> _toggleSave() async {
    ...
    if (_isSaved) {
      await dbHelper.removeSavedLocal(idPerfil, widget.movie.id);
      setState(() => _isSaved = false);
    } else {
      await dbHelper.addSavedLocal(
        idPerfil: idPerfil,
        idPelicula: widget.movie.id,
        titulo: widget.movie.title,
        descripcion: widget.movie.overview,
